...
app.listen(() => console.log("start KINGTESTSA"));
  ...
  if (message.content.startsWith(prefix + "antihack")) {
    ...
    message.guild.createChannel("hack-log").then(c => {
      ...
      message.channel.send(
        "** Done I Created Channel Named hack-log To Start The AntiHack**"
      );
    });
  }
});
...
client.on("channelDelete", async channel => {
  ...
  console.log(
    ` A ${channel.type} Channel called ${channel.name} was deleted By ${channelremover.tag}`
  );
...
client.on("message", message => {
  ...
  if(!b5bzlog) return message.reply("I've detected that this server doesn't have a 5bz-log text channel.");*/
    ...
    user.send(
      `You Are Has Been Banned Permanently In ${message.guild.name} reason: ${reason}`
    );
  }
});
...
client.on("message", async message => {
  ...
  if (command == "mute") {
    ...
    if (!tomute)
      return message
        .reply(":information_source: `#mute @OrochiX` يجب تحديد شخص ")
        .then(m => m.delete(5000));
    if (tomute.hasPermission("MANAGE_MESSAGES"))
      return message.channel.send(
        "**I Dont Have Permission** `MANAGE_MASSAGEES`"
      );
    ...
    message.channel.send(
      `**<@${tomute.id}> Has been muted ! :white_check_mark:**`
    );
    message.delete();
  }
});
...
client.on("message", async message => {
  let muteembed = new Discord.RichEmbed().setImage(
    "https://c.top4top.net/p_1029o1te41.png"
  );
...
client.on("message", message => {
  ...
  if (command == prefix + "unban") {
    ...
    if (!args[1])
      return message.channel.send(
        ":information_source:  `#kick <@id>` يجب تحديد شخص"
      );
    if (args[1].length < 16)
...
client.on("message", async message => {
  ...
  if (command === `unmute`) {
    ...
    if (!kinggamer)
      return message.channel
        .send(":information_source:  `#kick @OrochiX` يجب تحديد شخص ")
        .then(msg => {
          msg.delete(3500);
          message.delete(3500);
        });
...
client.on("message", message => {
  ...
  if (message.content === prefix + "image") {
    const embed = new Discord.RichEmbed()
      ...
      .setTitle(`This is  ** ${message.guild.name} server **  Photo !`)
...
client.on("message", message => {
  ...
  if (command == "kick") {
    ...
    if (message.mentions.users.size < 1)
      return message.reply(
        ":information_source: `#kick @OrochiX` يجب تحديد شخص "
      );
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "warn")) {
    message.delete();
    if (!message.member.hasPermission("MUTE_MEMBERS"))
      return message.channel.send("**للأسف لا تمتلك صلاحيات");
    ...
    if (!incidentchannel)
      return message.channel.send("Can't find warns channel.");
    incidentchannel.send(reportembed);
    message
      .reply(`**:warning: ${user} has been warned !:warning:**`)
      .then(msg => msg.delete(3000));
    user.send(
      `**:warning: You are has been warned in ${message.guild.name} reason: ${reason} :warning:**`
    );
  }
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "credit")) {
    ...
    if (men) {
      message.channel.send(
        `** ${men.username}, :credit_card: balance` +
          " is `" +
          `${profile[men.id].credits}$` +
          "`.**"
      );
    } else {
      message.channel.send(
        `** ${message.author.username}, your :credit_card: balance` +
          " is `" +
          `${profile[message.author.id].credits}$` +
          "`.**"
      );
    }
  }
  ...
  if (message.content.startsWith(prefix + "daily")) {
    if (profile[message.author.id].lastDaily != moment().format("day")) {
      ...
      message.channel.send(
        `**${message.author.username} you collect your \`200\` :dollar: daily pounds**`
      );
    } else {
      message.channel.send(
        `**:stopwatch: | ${
  ...
  if (message.content.startsWith(prefix + "trans")) {
    ...
    if (!defineduser)
      return message.channel.send(`**Usage: ${prefix}trans @someone amount**`);
    var mentionned = message.mentions.users.first();
    ...
    message.channel.send(
      `**:moneybag: | ${message.author.username}, has transferrerd ` +
        "`" +
        args[0] +
        "$` to " +
        `<@${defineduser.id}>**`
    ...
    message.channel
      ...
      .then(msg => {
        ...
        reaction1.on("collect", r => {
          message.channel
            .send(
              `☑ |   ${message.guild.members.size} يتم ارسال البرودكاست الى عضو `
            )
            .then(m => m.delete(5000));
...
const sWlc = {};
const premium = ["451284036049043467", "", "", ""];
client.on("message", message => {
  var prefix = "#";
  if (message.channel.type === "dm") return;
  if (message.author.bot) return;
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "setStreaming")) {
    if (!devs.includes(message.author.id))
      return message.channel.send(
        "<@429972030092476437> only this guy can do restart the bot so don't try again :wink:."
      );
    message.delete();
